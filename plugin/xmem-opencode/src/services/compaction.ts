...
import { xmemClient } from "./client.js";
...
export function createCompactionHook(
  ...
  async function fetchProjectMemoriesForCompaction(): Promise<string[]> {
    try {
      const result = await xmemClient.searchProjectMemories(tags.project, CONFIG.maxProjectMemories);
  ...
  async function saveSummaryAsMemory(sessionID: string, summaryContent: string): Promise<void> {
    ...
    try {
      const result = await xmemClient.ingest(
        `[Session Summary]\n${summaryContent}`,
        tags.project
      );
    ...
    await ctx.client.tui
      .showToast({
        body: {
          ...
          message: `Context at ${(usageRatio * 100).toFixed(0)}% - compacting with XMem context...`,
          variant: "warning",
          duration: 3000,
        },
      })
      ...
      await ctx.client.tui
        .showToast({
          body: {
            ...
            message: "Session compacted with XMem context. Resuming...",
            variant: "success",
            duration: 2000,
          },
        })
  ...
  async function handleSummaryMessage(sessionID: string): Promise<void> {
    ...
    async event({ event }: { event: { type: string; properties?: unknown } }) {
      ...
      if (event.type === "message.updated") {
        ...
        if (info.role === "assistant" && info.summary === true && info.finish) {
          await handleSummaryMessage(sessionID);
          return;
        }
