...
class PineconeVectorStore(BaseVectorStore):
    ...
    def __init__(
        ...
        if create_if_not_exists:
            ...
            existing_indexes: List[str] = [idx.name for idx in self._pc.list_indexes()]
    ...
    def add(
        ...
        is_valid, error = self.validate_embeddings(
            embeddings, expected_dimension=self._dimension
        )
        if not is_valid:
        ...
        for i in range(0, len(vectors), PINECONE_BATCH_SIZE):
            ...
            total_batches = (
                len(vectors) + PINECONE_BATCH_SIZE - 1
            ) // PINECONE_BATCH_SIZE

            logger.debug(
                f"Upserting batch {batch_num}/{total_batches} ({len(batch)} vectors)"
            )
    ...
    def update(
        ...
        new_values: List[float] = (
            embedding if embedding is not None else existing.values
        )

        # Start with copy of existing metadata
        # .copy() creates shallow copy to avoid mutating original
        new_metadata: Dict[str, Any] = (
            existing.metadata.copy() if existing.metadata else {}
        )
        ...
        self._index.upsert(
            vectors=[{"id": id, "values": new_values, "metadata": new_metadata}],
    ...
    def get(self, ids: List[str]) -> List[Dict[str, Any]]:
        ...
        fetch_result = self._index.fetch(ids=ids, namespace=self._namespace)
    ...
    def get_stats(self) -> IndexStats:
        ...
        return IndexStats(
            ...
            fullness_percentage=stats.index_fullness
